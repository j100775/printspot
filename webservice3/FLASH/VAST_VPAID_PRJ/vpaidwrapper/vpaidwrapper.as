package {
	...
	public class vpaidwrapper extends Sprite implements IVPAID {
		...
		function xmlDataFromParams(): void {
			...
			jsFullPath=xmlCampaignList.jsbasepath;
		...
		function injectJS(): void {
			pmi5Trace("sjfullPath="+jsFullPath);
				var js3: Array = [
				'var script3 = document.createElement("SCRIPT")',
				'script3.src = "'+ jsFullPath + 'js/vidaptivplayer.js"',
			...
			var js2: Array = [
				...
				'script2.src = "'+ jsFullPath + 'owa/modules/base/js/owa.custom.js"',
			...
			var js: Array = [
				...
				'script.src = "'+ jsFullPath + 'owa/modules/base/js/owa.tracker-combined-min.js"',
		...
		private function onCounterTimer(e:TimerEvent): void {
			...
			if(jsReady)
			{
				...
				pmi5Trace(" stop-remove-js timer ")
		...
		protected function resetAd(): void {
			...
			pmi5Trace(" resetAd initWidth,initHeight="+ int(initWidth) + "," + int(initHeight) + ","+ viewMode + "w=" + pmi5adMC.width + " h=" + pmi5adMC.height  + "\n");
			...
			//pmi5adMC.width = maxSwfWidth;
			//pmi5adMC.height = maxSwfHeight;
			
pmi5Trace(" set dimesnion resetAd after stage holderRatio=" + holderRatio + ",conR=" + contentRatio + "PMi5W=" + pmi5adMC.width + " pmi5fHe=" + pmi5adMC.height  + " \n"); 
			
			 //now the size is ok, lets center the thing
          // pmi5adMC.x =((initWidth - maxSwfWidth )/2);
         //  pmi5adMC.y = ((initHeight - maxSwfHeight)/2);
				
			var newPmi5:MovieClip = new MovieClip();
			newPmi5.width = 10;
			newPmi5.height = 10;
			...
			this.addChild(newPmi5);
			
          /*  if(holderRatio > contentRatio){
                pmi5adMC.width = initWidth;
				pmi5Trace(" holderRatio Round FixedScaleX=" + Math.round(Number((pmi5adMC.scaleX).toFixed(2))));
				pmi5adMC.scaleX = Math.round(Number((pmi5adMC.scaleX).toFixed(2)));
                pmi5adMC.scaleY = Math.round(Number((pmi5adMC.scaleX).toFixed(2)));
				
				newPmi5.width = initWidth;
				newPmi5.scaleX = newPmi5.scaleX;
				newPmi5.scaleY = newPmi5.scaleX;
				
            }else{
                pmi5adMC.height = initHeight;
				pmi5Trace(" contentRatio Round fixedScaleY=" + Math.round(Number((pmi5adMC.scaleY).toFixed(2))));
				pmi5adMC.scaleY = Math.round(Number((pmi5adMC.scaleY).toFixed(2)));
                pmi5adMC.scaleX =Math.round(Number((pmi5adMC.scaleY).toFixed(2)));
				
				newPmi5.width = initHeight;
				newPmi5.scaleY = newPmi5.scaleY;
				newPmi5.scaleX = newPmi5.scaleY;
            }*/
			
			 if(holderRatio < contentRatio){
                pmi5adMC.width = initWidth;
				pmi5Trace("xWidth21=" + pmi5adMC.scaleX + ", x=" + (pmi5adMC.scaleX).toFixed(2));
				pmi5adMC.scaleX =  pmi5adMC.scaleX;
                pmi5adMC.scaleY = pmi5adMC.scaleX;
				 pmi5adMC.x =0;
...
pmi5Trace(" \n initWidth=" + initWidth + ",pmi5adMC.width=" + pmi5adMC.width + "\ninitHeight="+ initHeight +",  pmi5adMC.height=" +   pmi5adMC.height + "\npmi5adMC.x="+ pmi5adMC.x + " pmi5adMC.y=" + pmi5adMC.y);
			
			pmi5Trace("\nstage22 width=" + stage.stageWidth + " H=" + stage.stageHeight);
       ...
       //     pmi5adMC.y = 0;
				...
				pmi5Trace("\nnewPmi5 SP Added- width=" + newPmi5.width + " H=" + newPmi5.height + " old w=" + pmi5adMC.width + " ,H=" + pmi5adMC.height);
			
				
		/*	
			var xNewPos:Number =  Math.round(((initWidth - pmi5adMC.width )/2));
			var yNewPos:Number = Math.round(((initHeight - pmi5adMC.height)/2));*/
			
			
			/*var xNewPos:Number =  Math.round(((pmi5adMC.width -initWidth )/2));
			var yNewPos:Number = Math.round(((pmi5adMC.height - initHeight )/2));
			*/
			/*pmi5Trace("newPos=" + xNewPos + ", y" + yNewPos);
			if(Number(xNewPos)>0)
			 pmi5adMC.x = xNewPos;
			else
			 pmi5adMC.x = 0;	
			...
			pmi5Trace("\n after7 pmi5adMC.x="+ pmi5adMC.x + " pmi5adMC.y=" + pmi5adMC.y);
			
			//log("-- stageR="+ Number((holderRatio).toFixed(2)) + ",conR=" + contentRatio + ",iW="+ Math.round(holderRatio) +", iH=" + Math.floor(holderRatio)+ " x="+ Math.ceil(contentRatio)+ ",y=" + Math.floor(contentRatio));
			
		//	pmi5Trace(" \nresetAd after holderRatio=" + holderRatio + ",conR=" + contentRatio + ",iW="+ Math.round(holderRatio) +", iH=" + Math.floor(holderRatio)+ " x="+ Math.ceil(contentRatio)+ ",y=" + Math.floor(contentRatio));
		...
		function forthQuarComplete(e: Event): void {
			...
			if(this.switchToNonlin)
			{
				pmi5adMC.hidePlayerBgVideo();
				pmi5Trace("- [in switchToNonlin HidePlayerVid]-");
			}
			else
			{
				pmi5Trace("--[else stopAd in forthQ]- ");
				//stopAd();
			}
			changeLinerToNonLinear();
			pmi5Trace("switchToNonlin=" +switchToNonlin);
