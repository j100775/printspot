package com.pmi5media.classes {
	...
	public class PlayerBgVideoMC extends MovieClip {
		...
		private function addedTS(e: Event): void {
			...
			bgVideo = new Shape();
				bgVideo.graphics.beginFill(0x000000,1);
				bgVideo.graphics.drawRect(0,0,AppConst.SCREEN_WIDTH,AppConst.SCREEN_HEIGHT);
				bgVideo.graphics.endFill();
				this.addChild(bgVideo);
			...
			vidPlayer.fullScreenTakeOver = false; // handling full screen 
			...
			objFullScr.addEventListener(MouseEvent.CLICK, onFullScr);
			objFullScr.x = 580;
			objFullScr.y = 280;
			this.addChild(objFullScr);
		...
		function setPosEvents(): void {
			...
			objBtnbar.x = AppConst.SCREEN_WIDTH / 2 - objBtnbar.width / 2;
			objBtnbar.y = AppConst.SCREEN_HEIGHT - (objBtnbar.height * 1.20);

			objPlayPause.x = objBtnbar.x - (objPlayPause.width * 1.70);
			...
			objMuteBtn.x = objsb.width + (objMuteBtn.width * 1.7);
		...
		public function loadnPlayVideo(pXml: XMLList): void {
			try {
				...
				if (pXml.isresize.length() > 0) {
					...
					resizeH = pXml.resizeheight;
					resizeW = pXml.resizewidth;
					resizeOffsetX = pXml.resizeoffsetx;
					resizeOffsetY = pXml.resizeoffsety;
				}
		...
		function vidComplete(e: fl.video.VideoEvent): void {
			...
			dispatchEvent(new Event("testvid", true));
			hideStopVideo();
