package com.pmi5media.classes {
	...
	public class CloseBtnMC extends MovieClip {
		...
		{
			if((int(pXml.offsetx)*AppConst.MULTIPLIER)>1100)
			txtClose.x =-70;
			
			if(
				(int(pXml.offsetx)*AppConst.MULTIPLIER)>
				(AppConst.SCREEN_WIDTH-(AppConst.CLOSE_BTN_W-8))){
					this.x = AppConst.SCREEN_WIDTH - (AppConst.CLOSE_BTN_W+2);
			}
			else{
				this.x = int(pXml.offsetx) * AppConst.MULTIPLIER;
			}
			
			if((int(pXml.offsety)*AppConst.MULTIPLIER)>
				(AppConst.SCREEN_HEIGHT-(AppConst.CLOSE_BTN_H-8))){
				this.y = AppConst.SCREEN_HEIGHT - (AppConst.CLOSE_BTN_H+4);
			}
			else
			{
				this.y = int(pXml.offsety) * AppConst.MULTIPLIER;
			}
