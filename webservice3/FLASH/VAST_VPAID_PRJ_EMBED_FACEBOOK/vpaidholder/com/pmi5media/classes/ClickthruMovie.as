package com.pmi5media.classes {
	...
	public class ClickthruMovie extends MovieClip {
		...
		public var toolTipHeight: int = 44;
		...
		function drawBorder(pW: int, pH: int): void {
			...
			txtBg.graphics.drawRoundRect(toolX, toolY, pW + 20, pH, 12, 12);
		...
		public function urlReq(pXml: XMLList): void {
			...
			_hideAfter = 0;
			imgW = 64; // re set below if found
			imgH = 64; // re set below if found
			...
			fromX = int(pXml.fromleft)*AppConst.MULTIPLIER;
			fromY = int(pXml.fromtop)*AppConst.MULTIPLIER;
			...
			if (pXml.iconwidth.length() > 0)
				imgW = int(pXml.iconwidth)*AppConst.MULTIPLIER;

			if (pXml.iconheight.length() > 0)
				imgH = int(pXml.iconheight)*AppConst.MULTIPLIER;
			...
			toolWidth = int(pXml.tipwidth)*AppConst.MULTIPLIER;
			toolHeight = int(pXml.tipheight)*AppConst.MULTIPLIER;
			toolbgColor = pXml.tipbgcolor;
			toolLineColor = pXml.tiplinecolor;
			tooltextColor = pXml.tiptextcolor;

			toolX = int(pXml.tipoffsetx)*AppConst.MULTIPLIER;
			toolY = int(pXml.tipoffsety)*AppConst.MULTIPLIER;
		...
		public function clickURL(evt: MouseEvent): void {
			if (enableClk) {
				if (widgetType == "widget") {
					...
					dispatchEvent(new Event(AppConst.EVENT_CLT_OPEN_PAGE,true));
				}
		...
		function onOver(evt: MouseEvent): void {
			...
			txtTooltip.width = toolWidth;
			TooltipText(tipText);
			txtTooltip.x = toolX + 12;
			txtTooltip.y = toolY + 12;
			txtTooltip.visible = true;
		...
		//add image on background, here pIcon is image path
